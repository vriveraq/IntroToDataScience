# GOALS: Understanding the binomial and normal distribution. Explore different plotting techniques. 
# DATA: Prostate and Titanic data sets. 

#########################Part 1#############################

#Q1.
#For this problem we will start with a simulation in order to find out
#how large size needs to be for the binomial distribution to be approximated by the normal distribution. 
 
#(a)Let's let p = 0.45, use the rbinom function to generate 500 observations(n = 500)
...
#(b) Now use the techniques described in class to improve graphs. 
...
# Q2. Why do you think the Data Life Cycle is crucial to understanding the opportunities
...
#Q3.Install and Load the library ElemStatLearn. Load the data prostate into R.

install.packages("ElemStatLearn")
library(ElemStatLearn)
data(prostate)

#(a) Print out the structure of the dataset using str()
str(prostate)

#(b) What are the names of the features in the dataset?
names(prostate)

#(c) The first eight variables are the our predictors. Subset the dataset with these eight variables and name it X.
X = prostate[,1:8]

#(d) Name the correlation matrix for X CorX and print it out.
CorX = cor(X)
print(CorX)

#(e) Find out the pair of variables with largest correlation
...
#(f) Use corrplot function from package corrplot to make a correlation plot 
install.packages("corrplot")
library(corrplot)
par(mfrow=c(1,1))
corrplot(CorX)

#(f) Create a new variable called age_group: 
...
#(g) Bonus: Can you try to make the same plot using ggplot?
library(ggplot2)
...
#4. Load the dataset "titanic.csv" you downloaded from moodle, omit the NA using
...
#a. First print out the str() of the dataset and think about which variable need to be converted to categorical.
str(data_titanic)

### The variable survived or not could be a good categorical variable instead of an integer.

#b.If Survived = 0, change it to "No", else change it to "Yes". Replace the "Yes/No" variable to original "Survived" Variable
...
#c. Make a histogram for the Age variable and fill the histogram by Survived Variable.
#The plot is similar to http://ggplot2.tidyverse.org/reference/geom_freqpoly-9.png
...
#d.) Make a mosaic plot with Sex, Pclass, Survived variables. Google is your friend if you find yourself stuck. 
library(vcd)
